<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-1">
            ...
            <div class="text-sm text-gray-600">
                {{ __('Untuk modul: ') . $lesson->module->title }}
            </div>
        </div>
    </x-slot>
        ...
        <div class="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-surface border border-gray-200 shadow-sm rounded-2xl overflow-hidden">
                ...
                <div class="p-6 sm:p-8">
                    ...
                    <form method="POST" action="{{ route('lessons.update', $lesson) }}" enctype="multipart/form-data" class="space-y-8">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <select name="type" id="type" class="mt-2 block w-full rounded-xl border-gray-200 bg-white shadow-sm focus:border-tertiary focus:ring-tertiary">
                                    <option value="video" {{ $lesson->type == 'video' ? 'selected' : '' }}>{{ __('Video') }}</option>
                                    <option value="text" {{ $lesson->type == 'text' ? 'selected' : '' }}>{{ __('Teks / Artikel') }}</option>
                                    <option value="pdf" {{ $lesson->type == 'pdf' ? 'selected' : '' }}>{{ __('Dokumen PDF') }}</option>
                                    <option value="doc" {{ $lesson->type == 'doc' ? 'selected' : '' }}>{{ __('Dokumen Word') }}</option>
                        ...
                        <div>
                            ...
                            <textarea id="content-editor-initial" rows="6" class="hidden">{{ $lesson->content }}</textarea>
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-text-dark">{{ __('Lampiran (Opsional)') }}</label>
                        ...
                        <div class="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <a href="{{ route('courses.modules.index', $lesson->module->course) }}" class="inline-flex items-center justify-center rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-semibold text-gray-700 shadow-sm hover:bg-gray-50">
                                {{ __('Batal') }}
                            </a>
    ...
    <script>
        (function () {
            ...
            const initialTextarea = document.getElementById('content-editor-initial');
            const hiddenInput = document.getElementById('content-hidden');
            const form = quillContainer.closest('form');
            const uploadUrl = @json(route('modules.lessons.editor-images', $lesson->module));
            ...
            const initQuill = () => {
                if (!quill) {
                    ...
                    quill.root.innerHTML = initialTextarea.value || '';
                }
            };
